...
# Writes DE translations as markdown files for Astro content collection.
...
for post_file in "${posts[@]}"; do
  ...
  translate_payload=$(jq -nc \
    ...
    '{
      model: $model,
      messages: [
        { role: "system", content: "You translate blog content from English to German. Respond ONLY with valid JSON, no markdown fences. Format: {\"title\": \"...\", \"description\": \"...\", \"body\": \"...\"}\nIMPORTANT: Keep all code blocks (```...```) and their content EXACTLY as-is. Do not translate code, variable names, or code comments inside fenced code blocks. Keep all URLs unchanged. No em dashes." },
        { role: "user", content: ("Translate the following blog post content from English to German. Keep the same tone and style. Keep ALL fenced code blocks (```language ... ```) completely unchanged including their language tags. Keep technical terms, code snippets, and URLs unchanged. No em dashes.\n\nTitle: " + $title + "\nDescription: " + $description + "\nBody:\n" + $body) }
      ],
      temperature: 0.3,
      max_tokens: 4000
    }')
  ...
  # Write DE markdown file with frontmatter
  cat > "${OUTPUT_DIR}/${slug}.md" <<MDEOF
---
title: "${title_de}"
description: "${desc_de}"
---
...
MDEOF
  ...
  echo "    Written ${OUTPUT_DIR}/${slug}.md"
